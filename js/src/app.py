...
# app.config["SQLALCHEMY_DATABASE_URI"] = "mysql+pymysql://root:root@localhost/gimnasio"
...
class Clientes(db.Model):  # Producto hereda de db.Model
    """
    ...
    def __init__(self, nombre, apellido, direccion, codpostal, localidad, telefono, mail, fechanac, sexo):
        """
        ...
        Args:
            ...
            codpostal (str): Codigo postal del cliente.
            localidad (str): Localidad del cliente.
            telefono (str): Telefono cliente.
            mail (str): Mail cliente.
            fechanac (str): Fecha nacimiento.
            sexo (str) : Sexo cliente.
        """
        self.nombre = nombre
...
class ClientesSchema(ma.Schema):
    ...
    """
    class Meta:
        fields = ("id", "nombre", "apellido", "direccion", "codpostal", "localidad", "telefono", "mail", "fechanac", "sexo")

producto_schema = ClientesSchema()  # Objeto para serializar/deserializar un producto
productos_schema = ClientesSchema(many=True)  # Objeto para serializar/deserializar múltiples productos
...
def get_Productos():
    ...
    """
    all_productos = Clientes.query.all()  # Obtiene todos los registros de la tabla de productos
...
def get_producto(id):
    ...
    Retorna un JSON con la información del producto correspondiente al ID proporcionado.
    """
    producto = Clientes.query.get(id)  # Obtiene el producto correspondiente al ID recibido
...
def create_producto():
    ...
    direccion = request.json["direccion"]  # Obtiene el stock del producto del JSON proporcionado
    codpostal = request.json["codpostal"]  # Obtiene la imagen del producto del JSON proporcionado
    localidad = request.json["localidad"]
    telefono = request.json["telefono"]
    mail = request.json["mail"]
    fechanac = request.json["fechanac"]
    sexo = request.json["sexo"]
    new_producto = Clientes(nombre, apellido, direccion, codpostal, localidad, telefono, mail, fechanac, sexo)  # Crea un nuevo objeto Producto con los datos proporcionados
...
def update_producto(id):
    ...
    Retorna un JSON con el producto actualizado.
    """
    producto = Clientes.query.get(id)  # Obtiene el producto existente con el ID especificado
    ...
    producto.codpostal = request.json["codpostal"]
    producto.localidad = request.json["localidad"]
    producto.telefono = request.json["telefono"]
    producto.mail = request.json["mail"]
    producto.fechanac = request.json["fechanac"]
    producto.sexo = request.json["sexo"]
